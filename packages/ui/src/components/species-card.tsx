...
import type { Species } from '@repo/types';
...
export const SpeciesCard: React.FC<SpeciesCardProps> = ({ species, viewMode = 'grid', lang, linkToFilter = false }) => {
    ...
    const coverImage = species.mainImage || 'https://www.iiap.gob.pe/img/logo-iiap.png';
    ...
    const onPlay = (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        // Event logic simplified for now
        window.dispatchEvent(new CustomEvent('play-audio', { detail: species }));
    };
