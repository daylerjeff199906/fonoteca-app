...
interface StatsChartProps {
    statsContent: {
        s1: { count: string; label: string; desc: string };
        s2: { count: string; label: string; desc: string };
        s3: { count: string; label: string; desc: string };
    };
    chartContent: {
        title_sm: string;
        title: string;
...
const data = [
    { name: 'Anfibios', value: 485, color: '#3b82f6' },
    { name: 'Aves', value: 132, color: '#8b5cf6' },
    { name: 'Murciélagos', value: 100, color: '#ec4899' },
    { name: 'Grillos', value: 56, color: '#14b8a6' },
    { name: 'Reptiles', value: 42, color: '#6366f1' },
];
            ...
            <div className="bg-[#050514] border border-[#1e1b4b] p-8 md:p-12 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col xl:flex-row gap-12 xl:gap-8 items-center lg:items-start">
                ...
                <div className="w-full xl:w-5/12 flex flex-col justify-between relative z-10 lg:pr-8">
                    <div>
                        ...
                        <h2 className="text-4xl md:text-5xl text-white font-light tracking-tight leading-tight mb-6 text-left">
                            {chartContent.title}
                        </h2>
                        <p className="text-indigo-200/60 text-lg font-light max-w-md mb-10 text-left">
                            {chartContent.desc}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-2 gap-8 mb-10">
                        {statsList.map((stat, i) => (
                            ...
                            >
                                ...
                                <p className="text-sm md:text-base text-gray-400 font-light leading-snug text-left">
                                    {stat.label}
                                </p>
                            </motion.div>
                        ))}
