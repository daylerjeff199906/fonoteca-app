...
export const Header: React.FC<HeaderProps> = ({ content, logoSrc, lang = "es" }) => {
    ...
    return (
        <>
            ...
            >
                <div className="container mx-auto px-6 flex justify-between items-center h-full">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <span className="block w-full h-0.5 transition-all group-hover:bg-accent-green bg-white"></span>
                            <span className="block w-3/4 h-0.5 ml-auto transition-all group-hover:bg-accent-green group-hover:w-full bg-white"></span>
                            <span className="block w-full h-0.5 transition-all group-hover:bg-accent-green bg-white"></span>
                        </button>
                    </div>
                </div>
            ...
            >
                <div className="absolute inset-0 bg-primary-dark/90 backdrop-blur-sm" onClick={toggleMenu}></div>
                <div className={`absolute right-0 top-0 h-full w-full md:w-1/2 lg:w-1/3 bg-white dark:bg-[#0f172a] text-primary-dark dark:text-white shadow-2xl transform transition-transform duration-500 delay-100 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
                    ...
                    <div className="relative z-10 h-full flex flex-col p-12 justify-center">
                        <button onClick={toggleMenu} className="absolute top-8 right-8 text-primary-dark dark:text-white hover:text-accent-green transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-8 h-8"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                        <nav className="space-y-6 text-3xl font-light">
                            {Object.entries(content).map(([key, label]) => (
                                <a key={key} href={getHref(key)} className="block hover:text-accent-green hover:translate-x-2 transition-all transition-colors" onClick={toggleMenu}>
                                    {label}
                                </a>
                            ))}
                        </nav>
